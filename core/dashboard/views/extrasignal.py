...
from ..helpers.helpers import paginate, choices, remove_csv, find_id, CSVLoader, upload_csv, platform_choices
from ..constants.constants import PER_PAGE, CSV_TEMP
from ...extensions import db

# --------------------- /dashboard/extrasignal/ : List of extrasignals ------------- #

@dashboard.route('/extrasignal/')
@login_required
def indexExtrasignal():
    ...
    return render_template('dashboard/extrasignal/index.html', extrasignals=extrasignals,start=start)

# ----------------------- /dashboard/extrasignal/id : Show extrasignal ------------------- #


@dashboard.route('/extrasignal/<int:id>', methods=['GET'])
@login_required
def showExtrasignal(id):
    extrasignal = Extrasignal.query.get_or_404(id)
    return render_template('dashboard/extrasignal/show.html', extrasignal=extrasignal)

# ---------------------- /dashboard/extrasignal/new : Add extrasignal -------------------- #


@dashboard.route('/extrasignal/new', methods=['GET', 'POST'])
@login_required
def newExtrasignal():
    """ Add new extrasignal """
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('dashboard.indexExtrasignal'))

    return render_template('dashboard/extrasignal/new.html', form=form)

#------------------------- /dashboard/extrasignal/bulk-add bulk add extra signals from file ------------------- #

@dashboard.route('/extrasignal/bulk-add', methods=['GET', 'POST'])
@login_required
def bulkAddExtraSignal():
    ...
    return render_template('dashboard/extrasignal/bulk.html', form=form, errors=errors)



# -------------------- /dashboard/extrasignal/id/edit : Edit extrasignal ----------------- #


@dashboard.route('/extrasignal/<int:id>/edit', methods=['GET', 'POST'])
@login_required
def editExtrasignal(id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('dashboard.showExtrasignal', id=id))

    return render_template('dashboard/extrasignal/edit.html', form=form, id=id)

# ------------------ /dashboard/extrasignal/id/delete : Delete extrasignal --------------- #


@dashboard.route('/extrasignal/<int:id>/toggle', methods=['GET'])
@login_required
...
@dashboard.route('/extrasignal/<int:id>/delete', methods=['GET'])
@login_required
...
# ------------------ /dashboard/extrasignal/search : extra signal lookup (by description or name)  --------------- #

@dashboard.route('/extrasignal/search', methods=['GET'])
@login_required
def searchExtraSignal():
    ...
    return render_template('dashboard/extrasignal/search.html', extrasignals=extrasignals)
