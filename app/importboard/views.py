...
importboard = Blueprint('importboard', __name__, url_prefix='/carboard/import')


# -------------------- /stream : Redis stream server ------------------------ #

@importboard.route('/stream')
def stream():
...
def index():
    ...
    return render_template('importboard/index.html')

# -------------------- /platform : Choose platform -------------------------- #


@importboard.route('/platform', methods=['GET', 'POST'])
@login_required
def platform():
    ...
    if request.method == 'POST':
        ...
        if wizard_platform is not False:
            ...
            return redirect(url_for('importboard.vehicle'))
        else:
            session['import'] = {}
            session.modified = True
            # session.clear();
    return render_template('importboard/platform.html', platforms=platforms)

# -------------------- /vehicle : Choose or add a vehicle ------------------- #


@importboard.route('/vehicle', methods=['GET', 'POST'])
@login_required
def vehicle():
    ...
    if check is True:
        ...
        if request.method == 'POST':
            ...
            if wizard_vehicle is not False:
                ...
                return redirect(url_for('importboard.driver'))
...
@importboard.route('/driver', methods=['GET', 'POST'])
@login_required
def driver():
    ...
    if check is True:
        ...
        if request.method == 'POST':
            ...
            if wizard_driver is not False:
                ...
                return redirect(url_for('importboard.records'))
        # # adding a new vehicle if any
...
@importboard.route('/records', methods=['GET', 'POST'])
@login_required
def records():
    ...
    if check is True:
        if request.method == 'POST':
            ...
            if files:
                ...
                try:
                    ...
                    # Import the module
                    mod = import_module(
                        ".indexing", 'app.importboard.platforms.' + slug)
                    # Indexing function
        ...
        return render_template('importboard/records.html')
    else:
        return check
...
@importboard.route('/getfile', methods=['POST'])
@login_required
def getfile():
    """ Import trace files"""
    if request.method == 'POST':
...
def process():
    ...
    if request.method == 'POST':
        ...
        return render_template('importboard/porcess.html')

# -------------------- /carboard/user/id : Show user ------------------------ #


@importboard.route('/help', methods=['GET'])
def help():
